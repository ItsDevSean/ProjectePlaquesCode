...
const Tooltip_vue_vue_type_script_setup_true_lang = require("./Tooltip.vue_vue_type_script_setup_true_lang-Dwvz6K4k.cjs");
...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    const vmodel = vue.computed(
      () => helpers.isTrueish(props.range) ? [minValue.value, maxValue.value] : valueStart.value || 0
    );
    vue.watch([valueStart, valueEnd], () => {
      if (helpers.isTrueish(props.range))
    ...
    const barSize = vue.computed(
      () => helpers.isTrueish(props.range) ? `${100 * (maxValue.value - minValue.value) / (props.max - props.min)}%` : `${100 * (valueStart.value - props.min) / (props.max - props.min)}%`
    );
    const barStart = vue.computed(
      () => helpers.isTrueish(props.range) ? `${100 * (minValue.value - props.min) / (props.max - props.min)}%` : "0%"
    );
    ...
    return (_ctx, _cache) => {
      return vue.openBlock(), vue.createElementBlock("div", {
        ...
        vue.createElementVNode("div", {
          ...
          vue.unref(helpers.isTrueish)(props.range) ? (vue.openBlock(), vue.createBlock(_sfc_main$2, {
            key: 1,
...
const index = {
  install(app) {
    ...
    plugins.registerComponent(app, _sfc_main$1);
  }
};
